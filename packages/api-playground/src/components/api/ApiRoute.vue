...
const methodDisplay = computed(() => (isWebSocket.value ? 'WS' : (props.route.method || 'UNKNOWN').toUpperCase()))
const methodClass = computed(() => (isWebSocket.value ? 'method-ws' : getMethodClass(props.route.method)))
...
<template>
  <div
    ...
    :class="[
      ...
      isTesting ? 'ring-1 ring-amber-400 shadow-[0_0_0_1px_rgba(217,119,6,0.3)] dark:ring-amber-600' : '',
    ]"
  ...
  >
    <div
      :class="[
        ...
        isExpanded ? 'bg-slate-50/90 dark:bg-slate-900/90' : 'hover:bg-slate-50/90 dark:hover:bg-slate-800/60',
      ]"
      ...
      <div class="flex items-center gap-3">
        ...
        <div class="min-w-0 flex-1">
          <div class="flex flex-wrap items-center gap-2">
            <span :class="['rounded border px-2 py-0.5 text-xs font-semibold tracking-wider flex-shrink-0', methodClass]">
              {{ methodDisplay }}
            </span>
          ...
          <p v-if="route.description" class="mt-1 truncate text-xs text-slate-500 dark:text-slate-400 line-clamp-1">
            {{ route.description }}
          </p>
        </div>
    ...
    <div v-show="isExpanded" class="route-details expanded border-t border-slate-200/80 px-4 py-4 dark:border-slate-700/80">
      <div class="grid gap-4 xl:grid-cols-[minmax(0,0.88fr)_minmax(0,1.12fr)]">
        <section class="space-y-3">
          <div class="rounded-xl border border-slate-200 bg-slate-50 p-3 dark:bg-slate-800/70 dark:border-slate-700">
            <h5 class="mb-2 text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">Operational details</h5>
            <div class="space-y-2 text-sm">
              <div v-if="route.middlewares?.length" class="break-words">
                ...
                <code class="ml-2 break-all text-orange-600 dark:text-orange-400 text-xs">{{ route.middlewares.join(', ') }}</code>
              </div>
              <div v-if="routeRateLimit" class="break-words">
                <span class="font-medium text-slate-700 dark:text-slate-200">Rate limit:</span>
                <span class="ml-2 rounded-full bg-rose-100 px-2 py-0.5 text-xs font-medium text-rose-700 dark:bg-rose-950/50 dark:text-rose-200">
                  {{ routeRateLimit.formatted }}
                </span>
              ...
              <div v-if="parameters.length">
                <h6 class="mb-1.5 text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">Parameters</h6>
                <div class="flex flex-wrap gap-1.5">
                  <div v-for="param in parameters" :key="param.name" class="rounded-xl border border-sky-200 bg-sky-50 px-2.5 py-1.5 text-xs dark:border-sky-800 dark:bg-sky-950/50">
                    <div class="font-semibold text-sky-700 dark:text-sky-200">{{ param.name }}</div>
                    <div class="mt-0.5 text-slate-500 dark:text-slate-400">{{ param.type }} · {{ param.required ? 'required' : 'optional' }}</div>
                  </div>
                </div>
              </div>
              <p v-if="!route.middlewares?.length && !routeRateLimit && !parameters.length" class="text-xs text-slate-400 dark:text-slate-500 italic">No additional details</p>
            </div>
          </div>

          <div class="rounded-xl border border-slate-200 bg-slate-50 p-3 dark:bg-slate-800/70 dark:border-slate-700">
            <h5 class="mb-2 text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">Query schema</h5>
            <div v-if="querySchema?.fields?.length" class="space-y-1.5">
              <div
                v-for="field in querySchema.fields"
                :key="field.name"
                class="border-l-2 border-emerald-200 pl-2.5 py-1 dark:border-emerald-700"
              >
                <div class="flex flex-wrap items-center gap-1.5 text-xs">
                  <span :class="['rounded px-1.5 py-0.5 font-mono font-semibold', getFieldTypeClass(field.type)]">
                    {{ field.name }}
                  </span>
                  <code class="rounded bg-slate-200 px-1.5 py-0.5 text-xs dark:bg-slate-700">{{ field.type }}</code>
                  <span :class="field.required ? 'text-red-500 dark:text-red-400' : 'text-slate-400 dark:text-slate-500'">
                    {{ field.required ? 'required' : 'optional' }}
                  </span>
                </div>
              </div>
            </div>
            <p v-else class="text-xs italic text-slate-400 dark:text-slate-500">No query schema defined</p>
          </div>

          <div class="rounded-xl border border-slate-200 bg-slate-50 p-3 dark:bg-slate-800/70 dark:border-slate-700">
            <h5 class="mb-2 text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">Request schema</h5>
            <div v-if="inputSchema?.fields?.length" class="space-y-1.5">
              <div
                v-for="field in inputSchema.fields"
                :key="field.name"
            ...
            <p v-else class="text-xs italic text-slate-400 dark:text-slate-500">No request schema defined</p>
          </div>
        </section>

        <section class="space-y-3">
          <div class="rounded-xl border border-slate-200 bg-slate-50 p-3 dark:bg-slate-800/70 dark:border-slate-700">
            <h5 class="mb-2 text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">Response schema</h5>
            <div v-if="outputSchema?.fields?.length" class="space-y-1.5">
              <div
                v-for="field in outputSchema.fields"
                :key="field.name"
              ...
              >
                ...
                >
                  ...
                  >
                    <span class="rounded bg-blue-50 px-1.5 py-0.5 font-mono font-semibold text-blue-700 dark:bg-blue-950/40 dark:text-blue-200">
                      {{ nestedField.name }}
                    </span>
                    <code class="rounded bg-slate-200 px-1.5 py-0.5 text-[11px] text-slate-700 dark:bg-slate-700 dark:text-slate-200">
                      {{ nestedField.type }}
                    </code>
                  </div>
                </div>
            ...
            <p v-else class="text-xs italic text-slate-400 dark:text-slate-500">No response schema defined</p>
          </div>

          <div class="rounded-xl border border-dashed border-slate-300 bg-white p-3 dark:border-slate-700 dark:bg-slate-900/50">
            <div class="flex items-center justify-between gap-3">
              <div>
                <h5 class="text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">Testing workspace</h5>
