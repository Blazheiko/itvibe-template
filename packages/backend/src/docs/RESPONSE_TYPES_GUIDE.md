...
All HTTP and WS error payloads must use the shared canonical envelope:
...
- Import the canonical schema/type from `shared/responses` instead of duplicating `code?/reason?/details?`.
- Use `type.or(successBranch, CanonicalErrorResponseSchema)` for shared response schemas.
- `code` is the shared machine-readable error code.
- `reason` is optional domain-specific detail.
- `details` is structured safe metadata only.
- Do not use legacy statuses such as `'unauthorized'` in API payloads.

### Example

```typescript
export const GetUserResponseSchema = type.or(
